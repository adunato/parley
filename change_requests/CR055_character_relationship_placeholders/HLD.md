...
- **GameAttribute Interface:**
  ...
  - Add an optional boolean field (e.g., `shareLastName?: boolean`) to explicitely define if placeholders generated under this attribute should take the main character's last name.
...
- During the `generateCharacter` flow, after the main character's `basicInfo` and `mappedAttributes` are resolved:
  ...
  - If the attribute belongs to the "Siblings" category (or a dynamically identified sibling category) and `relatedCharacterCount > 0`:
    ...
    - **Propagation Rules:** Same `originLocation`, same last name (parsed from main character's given name, or based on `shareLastName` if set), randomly selected age within `±10` years, random valid profession.
  - If the attribute belongs to the "Relationships" category and `relatedCharacterCount > 0`:
    ...
    - **Propagation Rules:** Randomly selected age within `±5` years. Keep `originLocation` in the same country/state. If `shareLastName` is true, the placeholder shares the last name.
