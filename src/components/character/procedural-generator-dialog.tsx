...
export function ProceduralGeneratorDialog({ open, onOpenChange, onApply, characterId }: ProceduralGeneratorDialogProps) {
    ...
    // Constants
    const SETTINGS_KEY = `parley_proc_gen_settings_${characterId}`;
    ...
    useEffect(() => {
        ...
        if (saved) {
            try {
                ...
                if (parsed.bioState) setBioState(parsed.bioState);
                if (parsed.generatedBioText) setGeneratedBioText(parsed.generatedBioText);
            } catch (e) {
    ...
    }, [hasLoaded, country, selectedState, town, selectedGender, age, mode, targetChildhood, targetProfessional, identity, bioState, generatedBioText, SETTINGS_KEY]);
                        ...
                        {identity && (
                            <div className="bg-muted p-4 rounded-lg border flex justify-between items-center">
                                <div>
                                    ...
                                    <div className="text-sm text-muted-foreground">{identity.gender}  {identity.location}</div>
                                </div>
